...
const UpdateApprovalSettingModal = ({ show, handleClose, approvalSetting }) => {
  const { data: users } = useGetActiveUsers();
  const { data: permissions } = useGetPermissions();
  const { mutateAsync, isPending } = useAddApprovalSetting();
  const defaultApprovalUser = approvalSetting.approval_setting_approvers.map(
    (approvalSetting) => {
      return {
        userId: approvalSetting.user_id,
        level: approvalSetting.level,
      };
    }
  );
  //   console.log(defaultApprovalUser);
  const {
    register,
    handleSubmit,
    getValues,
    ...
    defaultValues: {
      name: approvalSetting.name,
      permissionId: approvalSetting.permission_id,
      approvalLevel: approvalSetting.approval_level,
      action: approvalSetting.action,
      approvalUser: defaultApprovalUser,
    },
  });
...
export default UpdateApprovalSettingModal;
